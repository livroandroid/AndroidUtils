...
//                    mActivity, clickedView, transitionName);
    ...
    public void setOrAnimatePlusCheckIcon(final ImageView imageView, boolean isCheck,
        ...
        if (!(drawable instanceof AnimatedStateListDrawable)) {
            drawable = mActivity.getResources().getDrawable(drawableId /*R.drawable.add_schedule_fab_icon_anim*/);
            imageView.setImageDrawable(drawable);
        }
        imageView.setColorFilter(isCheck ? mActivity.getResources().getColor(colorFilterWhenChecked/*R.color.theme_accent_1*/) : Color.WHITE);
        if (allowAnimate) {
    ...
    public void compatSetOrAnimatePlusCheckIcon(final ImageView imageView, boolean isCheck,
                                                boolean allowAnimate, int button_icon_checkedId, int add_schedule_button_icon_uncheckedId) {

        final int imageResId = isCheck
                ? button_icon_checkedId
                : add_schedule_button_icon_uncheckedId;
